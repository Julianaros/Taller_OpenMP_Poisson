...
// Paralelización básica del bucle principal en solve_poisson
...
void create_data_directory() {
    try {
        if (!std::filesystem::exists("actividad1")) {
            std::filesystem::create_directory("actividad1");
...
// ACTIVIDAD 1: Función principal modificada con #pragma omp parallel for
int solve_poisson(Ejemplo ejemplo, std::vector<std::vector<double>>& V, 
    ...
    while (delta > TOL) {
        ...
        // PARALELIZACIÓN DEL BUCLE PRINCIPAL CON REDUCTION
...
void run_simulation(Ejemplo ejemplo, int M = 1000, int N = 1000) {
    ...
    std::cout << "\n=== RESULTADOS ACTIVIDAD 1: #pragma omp parallel for ===" << std::endl;
    ...
    if (config.solucion_analitica != "No disponible") {
        ...
        std::string analytical_filename = "ejemplo_" + std::to_string(static_cast<int>(ejemplo)) + "_parallel_for_analytical.dat";
        export_to_file(V_analytical, h, k, M, N, analytical_filename, config);
    }
    
    std::string numerical_filename = "ejemplo_" + std::to_string(static_cast<int>(ejemplo)) + "_parallel_for_numerical.dat";
...
int main() {
    std::cout << "=== ACTIVIDAD 1: SIMULADOR DE ECUACIÓN DE POISSON 2D (#pragma omp parallel for) ===" << std::endl;
