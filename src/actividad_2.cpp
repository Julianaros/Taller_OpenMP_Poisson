...
// Acelerar anidamiento de bucles con #pragma omp parallel for collapse(2)
...
void create_data_directory() {
    try {
        if (!std::filesystem::exists("actividad2")) {
            std::filesystem::create_directory("actividad2");
            std::cout << "Directorio 'actividad2' creado exitosamente." << std::endl;
        }
    } catch (const std::filesystem::filesystem_error& ex) {
        std::cerr << "Error al crear el directorio 'actividad2': " << ex.what() << std::endl;
    }
}
...
// ACTIVIDAD 2: Función principal modificada con #pragma omp parallel for collapse(2)
int solve_poisson(Ejemplo ejemplo, std::vector<std::vector<double>>& V, 
    ...
    while (delta > TOL) {
        ...
        // PARALELIZACIÓN CON COLLAPSE(2) PARA ACELERAR ANIDAMIENTO DE BUCLES
...
void export_to_file(const std::vector<std::vector<double>>& V, double h, double k, int M, int N, 
    ...
    std::string full_path = "actividad2/" + filename;
...
void run_simulation(Ejemplo ejemplo, int M = 50, int N = 50) {
    ...
    std::cout << "\n=== RESULTADOS ACTIVIDAD 2: collapse(2) ===" << std::endl;
    ...
    if (config.solucion_analitica != "No disponible") {
        ...
        std::string analytical_filename = "ejemplo_" + std::to_string(static_cast<int>(ejemplo)) + "_collapse2_analytical.dat";
        export_to_file(V_analytical, h, k, M, N, analytical_filename, config);
    }
    
    std::string numerical_filename = "ejemplo_" + std::to_string(static_cast<int>(ejemplo)) + "_collapse2_numerical.dat";
...
int main() {
    std::cout << "=== ACTIVIDAD 2: SIMULADOR DE ECUACIÓN DE POISSON 2D (collapse(2)) ===" << std::endl;
    std::cout << "Esta implementación utiliza #pragma omp parallel for collapse(2) reduction(max:delta)" << std::endl;
    std::cout << "para acelerar el anidamiento de bucles.\n" << std::endl;
    ...
    if (opcion >= 0 && opcion <= 4) {
        ...
        std::cout << "\n=== EJECUTANDO TODOS LOS EJEMPLOS CON collapse(2) ===\n" << std::endl;
        
        std::cout << "REFLEXIÓN SOBRE collapse(2):" << std::endl;
        std::cout << "• collapse(2) combina los dos bucles anidados en un solo bucle paralelo" << std::endl;
        std::cout << "• Aumenta el número total de iteraciones disponibles para paralelizar" << std::endl;
        std::cout << "• Mejora el balance de carga entre threads especialmente para grillas rectangulares" << std::endl;
        std::cout << "• No siempre es recomendable: puede aumentar overhead si los bucles son muy pequeños\n" << std::endl;
